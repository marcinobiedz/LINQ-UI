...
// Define a specific connection points for every shape
        ...
        erd.Relationship.prototype.getConnectionPoint = function (referencePoint) {
            ...
            const line = g.Line(bbox.center(), referencePoint);
            return (
...
// Bind custom ones.
        paper.on('cell:highlight', function (cellView) {
            ...
            const bbox = g.rect(cellView.getBBox({useModelGeometry: true})).moveAndExpand({
                x: -padding,
                y: -padding,
                width: 2 * padding,
                height: 2 * padding
...
// Create shapes
        ...
        const paid = new erd.IdentifyingRelationship({
        ...
        const employeeName = new erd.Normal({
        ...
        const skills = new erd.Multivalued({
        ...
        const uses = new erd.Relationship({
...
// Create new shapes by cloning
        ...
        const date = employeeName.clone().position(590, 80).attr('text/text', 'Date');
